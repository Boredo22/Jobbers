...
floor, comp above their ceiling, on-site when they need remote) is a weak fit no
...
- **baseCompUsd** — if the posting states a base salary or range, the base in USD
  as an integer (the LOWER end of a range, e.g. a "$140k–$180k" range → 140000).
  Null if no base pay is stated. This drives the app's comp-ceiling filter, so read
  it carefully from the posting text — but never guess a number the posting doesn't
  give.
